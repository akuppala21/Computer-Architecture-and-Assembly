...
int main(void)
{
    ...
    while(1) {
        ...
        int switch_value = (PORTD >> 8) & 0xF;
        timer_wait(switch_value);
        if (button1_pressed) {
            selected_cycle = (selected_cycle + 1) % 4;
        }
...
void timer_wait(int delay_multiplier)
 {
         const int DLY = 4000;
         int delay = DLY * delay_multiplier;
         if (delay == 0) {
             delay = DLY;
         }
